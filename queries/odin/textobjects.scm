...
(member_expression (call_expression)) @call.outer

; plain call
((_ (call_expression) @call.outer ) @_parent (#not-has-type? @_parent "member_expression")) 

; call arguments
((call_expression 
   function: (_) . argument: (_) @_first argument: (_) @_last . ) 
  (#make-range! "call.inner" @_first @_last))

; block
(block . "{" . (_) @_start @_end (_)? @_end . "}"  (#make-range! "block.inner" @_start @_end) ) @block.outer
...
((assignment_statement . (_) @_first (_) @_prelast . (_) @assignment.rhs @assignment.inner . ) 
  (#make-range! "assignment.lhs" @_first @_prelast)) @assignment.outer

; attribute
(attribute (_) @attribute.inner) @attribute.outer
...
((call_expression function: (_)
  "," @_start . argument: (_) @parameter.inner)
 (#make-range! "parameter.outer" @_start @parameter.inner)) 

((call_expression function: (_)
  . argument: (_) @parameter.inner . ","? @_end)
 (#make-range! "parameter.outer" @parameter.inner @_end)) 
